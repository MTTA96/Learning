...
+ (NSString*) makeRAPI_SOP_SignatureWithMid:(NSString*) mid andOrderId:(NSString*) order_id andPaymentType:(NSString*) payment_type andAmount:(NSString*)amount andCcy:(NSString*) ccy andSecretKey:(NSString*) secret_key andCardNumber:(NSString*) cardNumber andExpDate:(NSString*) expDate andCvv:(NSString*) cvv andPayer_id:(NSString*)payer_id {
    NSMutableString* key = [[NSMutableString alloc]init];
    if (mid) {
        [key appendString:mid];
    }
    ...
    if(payer_id && ![payer_id isEqualToString:@""]) {
        ...
        //        [key appendString:expDate];
        [key appendString:payer_id];
        if (cvv && ![cvv isEqualToString:@""] && cvv.length > 2) {
            [key appendString: [cvv substringFromIndex:cvv.length - 1]];
        }
