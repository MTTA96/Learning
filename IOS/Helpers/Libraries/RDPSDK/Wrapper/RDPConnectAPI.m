...
//    [self checkVarCompletion];
    ...
    NSString* signatureValue = [RDPSignatureUtil makeSignatureFromParams:dictionary withSecretKey:self.secret_key];
    [dictionary setValue:signatureValue forKey:@"signature"];
    
    NSString* queryString = [RDPQueryStringBuilder constructQueryString:dictionary];
    ...
    [[RDPHttpPostRequest alloc]initUsingHeader:NO andURL:url andDataString:queryString andResponseObject:[[RDPStringResponse alloc] init] andDelegate:self];
...
- (void) onSuccessWith:(RDPWsResponse*)responseObject {
    ...
    RDPStringResponse* response = (RDPStringResponse*) responseObject;
