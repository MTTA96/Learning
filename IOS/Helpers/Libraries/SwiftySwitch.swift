...
//  CHC
//
//  Created by Anh Tran on 7/24/18.
//
...
public class SwiftySwitch: UIView {
    ...
    @IBInspectable public var myOffColor: UIColor = UIColor(red:0.42, green:0.82, blue:0.9, alpha:1) {
        didSet {
            config()
        }
    }
    ...
    @IBInspectable public var dotTime: Double = 0.5 {
        didSet {
    ...
    @IBInspectable public var smallDotOffColor: UIColor = UIColor(red:0.44, green:0.46, blue:0.55, alpha:1) {
        didSet {
            config()
        }
    }
    ...
    private func config() {
        ...
        self.backgroundColor = isOn ? myColor : myOffColor
        self.layer.cornerRadius = myFrame.height * corners0to1
        var oldBall = ball
        ball = SwitchBall(dotOnColor, dotOffColor, smallDotColor, smallDotOffColor, isOn, myFrame, dotSpacer, smallDot0to1, dotTime)
        self.addSubview(ball)
        oldBall?.removeFromSuperview()
        oldBall = nil
        
        self.layer.borderWidth = isOn ? 0 : 2
        self.layer.borderColor = UIColor(red:0.44, green:0.46, blue:0.55, alpha:1).cgColor
...
fileprivate class SwitchBall: UIView {
    ...
    private var dotOnColor: UIColor!
    private var dotOffColor: UIColor!
    ...
    fileprivate init(_ onColor: UIColor, _ offColor: UIColor, _ dotColor: UIColor, _ dotOffColor: UIColor, _ isOn: Bool, _ myFrame: CGRect, _ dotSpace: Int, _ smallDotMultiplier: CGFloat, _ dotTravelTime: TimeInterval) {
        self.myFrame = myFrame
        self.smallDotMultiplier = smallDotMultiplier
        self.dotTravelTime = dotTravelTime
        self.dotOnColor = dotColor
        self.dotOffColor = dotOffColor
...
//        centerBall.backgroundColor = dotColor
        ...
        if isOn {
            ...
            centerBall.backgroundColor = dotOffColor
